...
const LowerBodyPage = () => {
  ...
  const [exercises, setExercises] = useState(null);  // Variable d'état pour stocker les exercices du programme bas du corps
    ...
    const fetchProgramData = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          setExercises(userData.programs.lowerBody.exercises); // Stocker les exercices du bas du corps
        }
      } catch (error) {
  ...
  const handleChange = (index, field, value) => {
    ...
    update(userRef, {
      ['programs/lowerBody/exercises/' + index + '/' + field]: value,
    });
  };
  ...
  const handleIncrement = (index, field) => {
    ...
    const userRef = ref(database, 'users/' + userId);
    update(userRef, {
      ['programs/lowerBody/exercises/' + index + '/' + field]: updatedExercises[index][field],
    });
  };
